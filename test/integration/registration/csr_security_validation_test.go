...
// testMaliciousCSRScenario is a helper function that sets up a cluster, approves it,
// then tests that a malicious CSR is NOT auto-approved
func testMaliciousCSRScenario(testName, clusterName, hubKubeconfigSecret string, csrCommonName string, csrOrgs []string, description string) {
	var err error

	hubKubeconfigDir := path.Join(util.TestDir, testName, "hub-kubeconfig")
	...
	cancel := runAgent(testName, agentOptions, commOptions, spokeCfg)
	defer cancel()
	...
	// Create the malicious CSR
	maliciousCSR := createMaliciousCSR(clusterName, csrCommonName, csrOrgs)
	...
	}, 10*time.Second, 1*time.Second).Should(gomega.BeTrue(), description)
