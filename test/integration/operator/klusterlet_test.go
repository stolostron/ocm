...
var _ = ginkgo.Describe("Klusterlet", func() {
	...
	ginkgo.Context("klusterlet statuses", func() {
		...
		ginkgo.It("should have correct available conditions", func() {
			...
			// Re-fetch objects before UpdateStatus to avoid stale resourceVersion conflicts
			gomega.Eventually(func() error {
				...
				_, err = kubeClient.AppsV1().Deployments(klusterletNamespace).UpdateStatus(context.Background(), deploy, metav1.UpdateOptions{})
				return err
			}, eventuallyTimeout, eventuallyInterval).Should(gomega.BeNil())
			gomega.Eventually(func() error {
