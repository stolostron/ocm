...
func TestAddonAnnotationsFromManagedCluster(t *testing.T) {
	...
	if annotations[addonv1alpha1.HostingClusterNameAnnotationKey] != "local-cluster" {
		t.Fatalf("expected hosting cluster name local-cluster, got %q",
			annotations[addonv1alpha1.HostingClusterNameAnnotationKey])
	}
	if annotations[addonv1alpha1.HostedManifestLocationAnnotationKey] != "hosting" {
		t.Fatalf("expected hosted manifest location hosting, got %q",
			annotations[addonv1alpha1.HostedManifestLocationAnnotationKey])
	}
...
func TestAddonAnnotationsFromManagedCluster_hostingClusterNamePrecedence(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := AddonAnnotationsFromManagedCluster(c.cluster)[addonv1alpha1.HostingClusterNameAnnotationKey]
			if got != c.expected {
...
func TestAddonAnnotationsFromManagedCluster_excludesNearMatchKeys(t *testing.T) {
	nearMatchKey := addonv1alpha1.GroupName + "-extra/near-match"
	cluster := &clusterv1.ManagedCluster{
		ObjectMeta: metav1.ObjectMeta{
			Name: "cluster1",
			Annotations: map[string]string{
